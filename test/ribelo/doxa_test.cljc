  ...
  (t/testing "where"
    ...
    (t/is (= `{~'_ {~'?e1 {:name "Ivan"}
             ...
             (-> (dx/parse-query '[:where
                                   ...
                                   [?e2 :name "Petr"]])
                 (dx/datalog->meander))))
    (t/is (= `(m/and {~'_ {~'?e {:name "Ivan" :age ~'?age}}}
                     (m/guard ~'(> ?age 18)))
             (-> (dx/parse-query '[:where
                                   [?e :name "Ivan"]
                                   [?e :age ?age]
  ...
  (t/testing "in & args"
    ...
    (t/is (= `{~'_ {~'?e {:name (m/or "Ivan" "Petr"), :age (m/or 20 30)}}}
             (-> (dx/parse-query '[:where
                                 ...
                                 [20 30])
                 (dx/datalog->meander))))
    (t/is (= `(m/and {~'_ {~'?e {:name (m/or "Ivan" "Petr")}}})
             (-> (dx/parse-query '[:where
                                   [?e :name ?name]
                                   [?e :age ?age]
                                   :in    [[?name ?age]]]
...
(comment
  (def db (dx/db-with [{:db/id 1, :name "Ivan", :age 15}
                       {:db/id 2, :name "Petr", :age 37}
                       {:db/id 3, :name "Ivan", :age 37}
                       {:db/id 4, :age 15}])))

(t/deftest test-joins
  (let [db (dx/db-with [{:db/id 1, :name "Ivan" :age 15}
                        {:db/id 2, :name "Petr" :age 37}
                        {:db/id 3, :name "Ivan" :age 37}
                        {:db/id 4, :age 15}])]
    (t/is (= [[1] [2] [3]]
  ...
  (let [expected #{[{:vehicle/brand "Aston Martin" :vehicle/model "DB5"}]
    ...
    (t/is (= expected
             (dx/q [:find  (pull [:vehicle/brand :vehicle/model] [?table ?e])
                    :where [?e :vehicle/brand "Aston Martin"]]
               bond-db)))))
