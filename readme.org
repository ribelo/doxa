...
comes to the operation of references and naming entity identifications.
=[person/id :ivan]= looks better than =:person.id/ivan=, easier to create and easier
to use.
...
to the possibility of limiting the iterated elements
...
(def dx-query-with-explict-table
  (enc/qb 1
    (d/q '[:find ?e
           :where
           [:fruid/id ?e :name "Avocado"]]
      dx100k)))
