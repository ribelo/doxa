...
1.  [rationale](#orgcac7e61)
2.  [db structure](#orgef9151b)
3.  [usage examples](#org02ac985)
    1.  [transactions](#org32c808d)
        1.  [put](#orga867e6e)
        2.  [delete](#org19186a6)
        3.  [conj](#orga228890)
        4.  [merge](#org0a56601)
        5.  [update](#org1719642)
        6.  [match](#org724fe91)
    2.  [with metadata](#orgcca33ff)
    3.  [pull](#orgf92d83a)
        1.  [eql](#org7b14550)
        2.  [datomic like pull syntax](#orgda0c843)
    4.  [datalog](#org7bc6507)
        1.  [joins](#org8958295)
    5.  [benchmark](#orgbe7199f)
        1.  [transaction](#org7f3fade)
        2.  [query](#org3b77630)
        3.  [pull](#org0ed8f52)

[![img](https://img.shields.io/clojars/v/com.github.ribelo/doxa.svg)](https://clojars.org/com.github.ribelo/doxa)

a simple in-memory database, trying to copy the best solutions from [datascript](https://github.com/tonsky/datascript),
[xtdb](https://github.com/xtdb/xtdb/), [fulcro](https://github.com/fulcrologic/fulcro), [autonormal](https://github.com/lilactown/autonormal) and especially [shadow-grove](https://github.com/thheller/shadow-experiments/blob/master/src/main/shadow/experiments/grove/db.cljc). each of them has its own
...
[re-frame](https://github.com/day8/re-frame) was one of the first solutions to propose one central `app-db`, one source of
...
to incorporate it into the `re-frame` ecosystem, eg. [posh](https://github.com/mpdairy/posh) and [re-posh](https://github.com/denistakeda/re-posh), and in my
...
tools with [re-frame-10x](https://github.com/day8/re-frame-10x) that allow you to preview `app-db` in real time.
...
variable `dx/*id-sufixes*`, if that wasn&rsquo;t enough. in addition it is possible to
explicitly declare via `metadata` which key is to be treated as the primary key of
an entity. examples in [metadata](https://github.com/ribelo/doxa#with-metadata).
...
also allowing you to split the data in a way that makes `q` queries [much faster](https://github.com/ribelo/doxa#can-we-be-faster-than-datascript-yes)
...
5.  caching & matching
    ...
    data that may have an impact. [editscript](https://github.com/juji-io/editscript) diffs are used, which, when converted
...
tables or assigned categories. so let&rsquo;s create 100k maps for 10 different
categories
